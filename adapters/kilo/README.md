...
- subagents: `~/.config/kilo/agents/<name>.md` (user) / `.kilo/agents/<name>.md` (project), rendered from `subagents/*.md` with `mode: subagent` and a `permission` block derived from the source `access`
