...
- project MCP config: `.mcp.json` at the project root, rendered from `mcps/*.json` into the `mcpServers` shape. Project-scope only and secret-free (values must use `${VAR}` placeholders); package-manager MCPs must use exact pinned package specs and are explicit manual network-bootstrap tools. `build` and project-scope/`--dest` installs write it, user-scope installs skip it as non-applicable. The user-global Claude MCP config is never auto-merged, and install blocks rather than clobbers a pre-existing `.mcp.json`.
