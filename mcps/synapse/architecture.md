...
mcps/synapse/
  ...
  install.mjs           - Build + link bins; deploy/restart launchd sidecar on macOS (npm run install:synapse).
...
| Lifecycle | bridge autostart default; launchd always-on via `install.mjs` | `IMPLEMENTED` | zero-setup default; always-on service | launchd-required (heavy default) |
...
| Bridge cwd ≠ workspace | wrong project namespace | resolved | bridge roots routing (`test/roots-routing.test.ts`: env → roots → cwd) |
