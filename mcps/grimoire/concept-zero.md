...
> ⚠️ **HISTORICAL HLD — do NOT implement from the body below.** Source of truth is **[architecture.md](architecture.md)** + **[roadmap.md](roadmap.md)** + **[api-contract.md](api-contract.md)**. Keep this doc for problem framing and the evidence ledger; the shipped server serves three registry packs (`anthropic-cybersecurity-skills`, `rev-skills`, `hack-skills`) and the "defer from native catalogs" precondition is enforced by `agent-surface check`.
...
| C-02 | constraint | The pack must be **deferred from all native skill catalogs**; grimoire is the sole path | USER_DECISION (enforced by `check`) |
